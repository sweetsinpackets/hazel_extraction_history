...
and uhtyp_seq_trans = (~t : Seq.t('operand, 'operator)) : extract_t =>        
    switch(t){   //I don't think oprand is necessary
        | S(operand, affix_e) => switch(affix_e){
            ...
            | A(operator, seqt) => uhtyp_const(~ope1=operand, ~op=operator, ~ope2=uhtyp_seq_trans(~t=seqt))
        }
    }
and uhtyp_operand_trans = (~ope : UHTyp.operand) : extract_t =>
    switch(ope){
        ...
        | Num => (Some("int"), Number)
        | Bool => (Some("bool"), Bool)
        | Parenthesized(t) => extract_t_concat(~le=[(Some("("), UNK), uhtyp_trans(~t=t), (Some(")"), UNK)]) 
        | List(t) => extract_t_concat(~le=[uhtyp_trans(~t=t), (Some(" list"), UNK)])
    }
and uhtyp_const = (~ope1:UHTyp.operand, ~op:UHTyp.operator, ~ope2:extract_t) : extract_t => {
    let (s1, p1) = uhtyp_operand_trans(~ope=ope1);
    let (s2, p2) = ope2;
    switch(op){
        | Arrow => (option_string_concat(~strs=[s1, Some("->"), s2]), ARROW(p1, p2))
        | Sum => (option_string_concat(~strs=[s1, Some("|"), s2]), SUM(p1, p2))
        | Prod => (option_string_concat(~strs=[s1, Some("*"), s2]), PROD(p1, p2))
    }
};
